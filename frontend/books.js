...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    if (file.fieldname === 'coverImage') {
      cb(null, path.join(__dirname, '..', process.env.UPLOAD_COVER_DIR)); // تأكد من تعريف UPLOAD_COVER_DIR في ملف .env
    } else if (file.fieldname === 'downloadLink') {
      cb(null, path.join(__dirname, '..', process.env.UPLOAD_DOWNLOAD_DIR)); // تأكد من تعريف UPLOAD_DOWNLOAD_DIR في ملف .env
    }
  },
